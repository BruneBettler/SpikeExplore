...
def compute_PCA(waveforms, all_chans=False, n_components=3):
    pca = PCA(n_components=n_components)
    spike_num, chan_num, sample_num = waveforms.shape

    if all_chans:
        X = waveforms.reshape(spike_num, -1)
        pc_features = pca.fit_transform(X)
        return pc_features, pca
    else:
        pc_features = np.empty((spike_num, chan_num, n_components))
        pcas = []

        for ch in range(chan_num):
            X_ch = waveforms[:, ch, :]          # (n_spikes, n_samples)
            pca = PCA(n_components=n_components)
            pc_features[:, ch, :] = pca.fit_transform(X_ch)
            pcas.append(pca)
    return pc_features, pcas


def firing_rate(spike_times, duration_s):
    return len(spike_times) / duration_s

def presence_ratio(spike_times, duration_s, sample_rate=20000, num_bin_edges=101):
    
    st = spike_times / sample_rate
    h, b = np.histogram(st, np.linspace(0, duration_s, num_bin_edges))

    return np.sum(h > 0) / (num_bin_edges - 1)

def isi_violations(spike_times, sample_rate=20000, threshold_s=0.0015):
    st = spike_times / sample_rate
